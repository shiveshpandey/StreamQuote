...
public class StreamingQuoteParserFactory {
	...
	public static IStreamingQuoteParser getStreamingQuoteParser() {
		...
		if (ZStreamingConfig.getStreamingQuoteMode().equals(
				ZStreamingConfig.QUOTE_STREAMING_MODE_LTP)) {
			streamingQuoteParser = new StreamingQuoteParserModeLtp();
		} else if (ZStreamingConfig.getStreamingQuoteMode().equals(
				ZStreamingConfig.QUOTE_STREAMING_MODE_QUOTE)) {
			streamingQuoteParser = new StreamingQuoteParserModeQuote();
		} else if (ZStreamingConfig.getStreamingQuoteMode().equals(
				ZStreamingConfig.QUOTE_STREAMING_MODE_FULL)) {
			...
			System.out
					.println("StreamingQuoteParserFactory.getStreamingQuoteParser(): ERROR: "
							...
							+ ZStreamingConfig.getStreamingQuoteMode() + "]");
		}
