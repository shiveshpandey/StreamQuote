...
public class StreamingQuoteParserThread implements Runnable {
	...
	public StreamingQuoteParserThread(BlockingQueue<Object> quoteBufferQ,
		...
		// get quote parser
		streamingQuoteParser = StreamingQuoteParserFactory
				.getStreamingQuoteParser();
		if (streamingQuoteParser == null) {
			System.out
					.println("StreamingQuoteParserThread.StreamingQuoteParserThread(): ERROR: Quote Parser is null... !!!");
		}
			...
			try {
				...
				if (bufferObj instanceof ByteBuffer) {
					...
					try {
						...
						// require this data
						System.out
								.println("StreamingQuoteParserThread.run(): ERROR: Exception in parsing the Buffer, reason: "
										+ e.getMessage());
					}
				}
			} catch (InterruptedException e) {
				System.out
						.println("StreamingQuoteParserThread.run(): ERROR: InterruptedException on take from quoteBufferQ");
				e.printStackTrace();
			}
		}
	}
	...
	private void parseBuffer(ByteBuffer buffer, String time) {
		...
		if (buffLen == 1) {
			...
			if (ZStreamingConfig.isHeartBitMsgPrintable()) {
				System.out
						.println("StreamingQuoteParserThread.parseBuffer(): WS HEARTBIT Byte");
			}
		} else {
			// num of Packets
			...
			if (numPackets == 0) {
				...
				System.out
						.println("StreamingQuoteParserThread.parseBuffer(): ERROR: WS Byte numPackets is 0 in WS message, Ignoring !!!");
			} else {
				start += 2;
				...
				for (int i = 0; i < numPackets; i++) {
					...
					if (numBytes != 0) {
						...
						if (pktBuffer != null) {
							...
							// - ignore
							System.out
									.println("StreamingQuoteParserThread.parseBuffer(): ERROR: pktBuffer is null in WS message, Ignoring !!!");
						}
					} else {
						// Invalid Case: Zero Num of Bytes in packet - ignore
						System.out
								.println("StreamingQuoteParserThread.parseBuffer(): ERROR: numBytes is 0 in WS message packet["
										+ i + "], Ignoring !!!");
					}
				}
			}
	...
	private void parseQuotePktBuffer(ByteBuffer pktBuffer, String time) {
		...
		if (ZStreamingConfig.isStreamingQuoteStoringRequired()
				&& (streamingQuoteStorage != null) && streamingQuote != null) {
