...
public class ZStreamingQuoteControl {
	...
	private static final String QUOTE_STREAMING_TIME = ZStreamingConfig
			.getStreamingQuoteStartTime();
	...
	private ZStreamingQuoteControl() {
		...
		if (ZStreamingConfig.isStreamingQuoteStoringRequired()) {
			streamingQuoteStorage = StreamingQuoteStorageFactory
					.getStreamingQuoteStorage();
			if (streamingQuoteStorage == null) {
				System.out
						.println("ZStreamingQuoteControl.ZStreamingQuoteControl(): ERROR: Streaming Quote storage is null... !!!");
			}
		} else {
			System.out
					.println("ZStreamingQuoteControl.ZStreamingQuoteControl(): WARNING: Streaming Quote Storage Config disabled");
		}
	}
	 ...
	 * @param publicToken
	 */
	public void start(final String apiKey, final String userId,
			final String publicToken) {
		if (ZStreamingConfig.isStreamingQuoteStartAtBootup()) {
			// start Streaming Quote WebSocket - immediately
			System.out
					.println("ZStreamingQuoteControl.start(): Starting Streaming Quote WS");
			startStreamingQuote(apiKey, userId, publicToken);
		} else {
			// start Streaming Quote WebSocket - at market open
			Thread t = new Thread(new Runnable() {
				private boolean runnable = true;
				private DateFormat dtFmt = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss");
				private TimeZone timeZone = TimeZone.getTimeZone("IST");
				private String refTime = todaysDate + " "
						+ QUOTE_STREAMING_TIME;
				...
				public void run() {
					...
					try {
						...
						while (runnable) {
							Date timeNow = Calendar.getInstance(timeZone)
									.getTime();
							if (timeNow.compareTo(timeRef) >= 0) {
								System.out
										.println("ZStreamingQuoteControl.start().StreamingQuoteStartThread.run(): Starting Streaming Quote WS at: "
												+ timeNow);
								runnable = false;
								startStreamingQuote(apiKey, userId, publicToken);
							}
						...
						System.out
								.println("ZStreamingQuoteControl.start(): ParseException while parsing time: ["
										+ refTime
										+ "], cause: "
										+ e1.getMessage());
					} catch (InterruptedException e) {
						System.out
								.println("ZStreamingQuoteControl.start(): InterruptedException for Thread sleep");
					}
				}
			});
			t.start();
	...
	/**
	 * getOHLCDataByTimeRange - public method to get OHLC Data between a time
	 * range
	 * 
	...
	public OHLCquote getOHLCDataByTimeRange(String instrumentToken,
		...
		if (ZStreamingConfig.isStreamingQuoteStoringRequired()
				&& (streamingQuoteStorage != null)) {
			// Get OHLC Data
			return streamingQuoteStorage.getOHLCDataByTimeRange(
					instrumentToken, fromTime, toTime);
		} else {
			return null;
		}
	}
	...
	/**
	 * getQuoteListByTimeRange - public method to get complete Streamed Data
	...
	public List<StreamingQuote> getQuoteListByTimeRange(String instrumentToken,
		...
		if (ZStreamingConfig.isStreamingQuoteStoringRequired()
			...
			return streamingQuoteStorage.getQuoteListByTimeRange(
					instrumentToken, fromTime, toTime);
		} else {
			return null;
		}
	 ...
	 * @param publicToken
	 */
	private void startStreamingQuote(String apiKey, String userId,
			String publicToken) {
		String URIstring = STREAMING_QUOTE_WS_URL_TEMPLATE + "api_key="
				+ apiKey + "&user_id=" + userId + "&public_token="
				+ publicToken;

		DateFormat quoteTableDtFmt = new SimpleDateFormat("ddMMyyyy");
		quoteTableDtFmt.setTimeZone(timeZone);
		String date = quoteTableDtFmt.format(Calendar.getInstance(timeZone)
				.getTime());
		...
		if (streamingQuoteStarted) {
			...
			System.out
					.println("ZStreamingQuoteControl.startStreamingQuote(): ERROR: WebSocket Streaming Quote not started !!!");
		}
	}
	...
	private void stopStreamingQuote() {
		...
		if (streamingQuoteStarted && websocketThread != null) {
			...
			try {
				...
				System.out
						.println("ZStreamingQuoteControl.stopStreamingQuote(): ERROR: InterruptedException while sleeping");
			}
		}
	...
	/**
	 * getInstrumentsList - private method to get Instruments List for streaming
	 * quotes
	 * 
	...
	private List<String> getInstrumentTokensList() {
		...
		System.out
				.println("ZStreamingQuoteControl.getInstrumentTokensList(): Subscribe Instrument Tokens: "
						+ instrumentList);
