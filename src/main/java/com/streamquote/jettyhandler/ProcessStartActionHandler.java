...
public class ProcessStartActionHandler extends ContextHandler {
	...
	public void doHandle(String target, Request baseRequest,
		...
		if (reqMethod.equals("GET")) {
			...
			if (ZStreamingConfig.isWebServiceLogsPrintable()) {
				System.out
						.println("ProcessStartActionHandler.doHandle(): ZStreamingQuote Process [GET]: START initiated"
								+ " ["
			...
			// Start the process
			String apiKey = queryValMap.get(ZStreamingConfig
					.getJettyServerProcessStartApiKeyReqParam());
			String userId = queryValMap.get(ZStreamingConfig
					.getJettyServerProcessStartUserIdReqParam());
			String publicToken = queryValMap.get(ZStreamingConfig
					.getJettyServerProcessStartPubTokenReqParam());
			if (ZStreamingConfig.isWebServiceLogsPrintable()) {
				System.out
						.println("ProcessStartActionHandler.doHandle(): DEBUG: apiKey: ["
								+ apiKey
								+ "], userId: ["
								+ userId
			...
			if (!TradingHolidays.isHoliday()) {
				...
				System.out
						.println("ProcessStartActionHandler.doHandle(): ERROR: Holiday Today....HALT Process !!!");
			}
			...
			System.out
					.println("ProcessStartActionHandler.doHandle(): ERROR: Request method not proper: "
							+ reqMethod);
		}
	...
	private Map<String, String> getQueryParameters(HttpServletRequest request) {
		...
		if (StringUtils.isEmpty(queryString)) {
			System.out
					.println("ProcessStartActionHandler.getQueryParameters(): ERROR: query string is empty !!!");
			return null;
		}
