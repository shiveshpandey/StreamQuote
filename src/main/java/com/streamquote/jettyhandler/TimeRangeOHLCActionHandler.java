...
public class TimeRangeOHLCActionHandler extends ContextHandler {
	...
	public void doHandle(String target, Request baseRequest,
		...
		String dataFormat = queryValMap.get(ZStreamingConfig
				.getJettyServerTimeRangeOHLCformatReqParam());

		// Get Time Range requested
		String fromTime = queryValMap.get(ZStreamingConfig
				.getJettyServerTimeRangeOHLCfromTimeReqParam());
		...
		String instrumentToken = queryValMap.get(ZStreamingConfig
				.getJettyServerTimeRangeOHLCinstrumentReqParam());

		if (reqMethod.equals("GET")) {
			// GET method
			if (ZStreamingConfig.isWebServiceLogsPrintable()) {
				System.out
						.println("TimeRangeOHLCActionHandler.doHandle(): ZStreamingQuote Time Range OHLC [GET]: Requested Format: "
								+ "["
								+ dataFormat
								+ "] fromTime: ["
								+ fromTime
			...
			OHLCquote quote = ZStreamingQuoteControl.getInstance()
					.getOHLCDataByTimeRange(instrumentToken, fromTime, toTime);
			String outData = null;
			if (quote == null) {
				 ...
				 * formatQuoteToCSV(quote); }
				 */else {
					outData = "<h1>Requested Format " + dataFormat
							+ " NOT supported, only csv or json</h1>";
					System.out
							.println("TimeRangeOHLCActionHandler.doHandle(): ERROR: ["
			...
			System.out
					.println("TimeRangeOHLCActionHandler.doHandle(): ERROR: Request method not proper: "
							+ reqMethod);
		}
	...
	private String formatQuoteToJSON(OHLCquote quote) {
		...
		try {
			...
			System.out
					.println("TimeRangeOHLCActionHandler.formatQuoteToJSON(): ERROR: JsonProcessingException on quote !!!");
			e.printStackTrace();
		}
	...
	private String formatQuoteToCSV(OHLCquote quote) {
		...
		CsvSchema schema = mapper.schemaFor(OHLCquote.class).withHeader()
				.withColumnSeparator(',');
		...
		try {
			...
			System.out
					.println("TimeRangeOHLCActionHandler.formatQuoteToCSV(): ERROR: JsonProcessingException on quote");
			e.printStackTrace();
		}
	...
	private Map<String, String> getQueryParameters(HttpServletRequest request) {
		...
		if (StringUtils.isEmpty(queryString)) {
			System.out
					.println("TimeRangeOHLCActionHandler.getQueryParameters(): ERROR: query string is empty !!!");
			return null;
		}
